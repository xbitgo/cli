...
func (m *Manager) SDI(pkg string, xst parser.XST) error {
...
func (m *Manager) DI(iParser *parser.IParser) error {
	gdi := tpls.DI{
		Pkg:  iParser.Package,
	...
	filename := fmt.Sprintf("%s/%s", iParser.Pwd, "di_register_gen.go")
	buf, err := gdi.Execute()
	if err != nil {
		return err
	}
