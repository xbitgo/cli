...
func (m *Manager) dbConv(xst parser.XST, gdo tpls.Do) error {
	convGen := tpls.DoConv{
		...
		Imports: []string{
			...
			"github.com/xbitgo/core/tools/tool_slice",
			fmt.Sprintf("%s/apps/%s/domain/entity", conf.Global.ProjectName, m.AppName),
			fmt.Sprintf("%s/apps/%s/repo_impl/do", conf.Global.ProjectName, m.AppName),
		},
		Fields: gdo.Fields,
	}
